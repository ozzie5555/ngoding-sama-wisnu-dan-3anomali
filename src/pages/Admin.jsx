  ...
  return <main className="admin-page">
    <div className="admin-shell">
      ...
      <section className="admin-content">
        ...
        {section === 'users' && <section className="admin-panel"><div className="admin-panel-header"><div><h2>Daftar Pengguna</h2><p>Role dan identitas akun yang terdaftar.</p></div><button type="button" className="admin-refresh-btn" onClick={loadUsers}>↻ Refresh</button></div>{usersLoading ? <div className="admin-empty"><span className="admin-spinner"/>Memuat pengguna...</div> : <div className="admin-users-list">{users.map((person) => <div className="admin-user-row" key={person.id}><div className="admin-user-avatar"><span aria-hidden="true">{(person.full_name || person.username || 'P').charAt(0).toUpperCase()}</span>{person.avatar_path && <img src={person.avatar_path} alt={`Foto ${person.full_name || person.username || 'pengguna'}`} onError={(event) => event.currentTarget.remove()}/>}</div><div className="admin-user-info"><strong>{person.full_name || 'Tanpa nama'}</strong><span>{person.username || person.email || 'Email tidak tersedia'}</span></div><span className={`admin-role-badge role-${person.role}`}>{person.role}</span><small>{person.phone || 'Telepon belum diatur'}</small></div>)}</div>}</section>}
      </section>
    </div>
